    ...
    rows = [
        {
            ...
            "description": subitem.get('description') or subitem.get('product_name', ''),
            "cost": subitem.get('unit_price', '').replace(',', '') if subitem.get('unit_price') else '',
...
def marvin_extract(text):
    ...
    # Regular expression to match lines with quantity and price
    # Updated pattern to better match quantity and price format
    pattern = re.compile(r'^\s*(\d+)\s+(?:EA|EACH)\s+(.+?)\s+\$([\d,]+\.\d{2})$')
    ...
    while i < len(lines):
        ...
        if match:
            ...
            unit_price = "{:.2f}".format(price_value / quantity)
            ...
            while i < len(lines):
                ...
                if len(parts) == 2:
                    ...
                    if price_match:
                        ...
                        # Check if unit_price is zero before division
                        if float(unit_price.replace(',', '')) != 0:
                            calculated_quantity = int(float(price.replace(',', '')) / float(unit_price.replace(',', '')))
                        else:
                            calculated_quantity = 0  # or handle this case as needed
...
def tmcobb_extract(text):
    ...
    pattern = re.compile(r'(\d+)\s+([\d,]+\.\d{2})\s+\$([\d,]+\.\d{2})$')
    ...
    while i < len(lines):
        ...
        if match:
            # Get the full line before the numbers as description
            full_line = line[:match.start()].strip()
            
            # Skip if line contains tax/total/etc
            skip_terms = ['tax', 'total', 'subtotal', 'distributed by', 'date']
            if any(term in full_line.lower() for term in skip_terms):
                i += 1
                continue
            ...
            # Get description from previous line if it exists
            description = ""
            if i > 0:
                prev_line = lines[i-1].strip()
                if prev_line and not any(term in prev_line.lower() for term in skip_terms):
                    description = prev_line
            
            # Add current line description
            if full_line:
                if description:
...
def upload_file():
    if 'file' not in request.files or 'extraction_type' not in request.form:
        return jsonify({'error': 'No file or extraction type provided'}), 400
    ...
    # Choose the extraction function based on the extraction type
    if extraction_type == 'Fleetwood':
        subitems = fleetwood_extract(extracted_text)
    elif extraction_type == 'Marvin':
        subitems = marvin_extract(extracted_text)
    elif extraction_type == 'TruStile':
        subitems = trustile_extract(extracted_text)
    elif extraction_type == 'TM Cobb':
        subitems = tmcobb_extract(extracted_text)
    else:
        return jsonify({'error': 'Invalid extraction type'}), 400

    # Convert subitems to JSON format
    rows = convert_to_json(subitems)
    print(rows)
